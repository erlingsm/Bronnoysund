# bronnoysund-lookup — Helm chart

Helm chart for the `Bronnoysund.Lookup.WebApi` service. Works on any standard Kubernetes
...
```bash
helm install bronnoysund ./charts/bronnoysund-lookup \
...
```bash
helm install bronnoysund ./charts/bronnoysund-lookup --values my-values.yaml
```

### 3. Verify

```bash
kubectl rollout status deployment/bronnoysund-bronnoysund-lookup
kubectl port-forward svc/bronnoysund-bronnoysund-lookup 8080:80
...
```bash
helm upgrade --install bronnoysund ./charts/bronnoysund-lookup \
...
helm template bronnoysund ./charts/bronnoysund-lookup

# Lint
helm lint ./charts/bronnoysund-lookup

# Dry-run install against a real cluster
helm install bronnoysund ./charts/bronnoysund-lookup --dry-run --debug

# Test against a kind cluster
kind create cluster --name brreg
helm install bronnoysund ./charts/bronnoysund-lookup --set image.pullPolicy=IfNotPresent
kubectl port-forward svc/bronnoysund-bronnoysund-lookup 8080:80
```
...
kubectl delete pvc bronnoysund-bronnoysund-lookup-data
```
...
Bronnoysund.Lookup uses Container Apps as the default deployment because it fits the
