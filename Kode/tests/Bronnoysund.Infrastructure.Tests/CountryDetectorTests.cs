...
public class CountryDetectorTests
{
    ...
    [InlineData("PL7740001454")] // With PL VAT prefix — iter-3 caught this was uncovered
    [InlineData("pl 7740001454")] // mixed-case prefix
    ...
    public void Detect_PolishRegon9Digit_ReturnsRegon(string raw)
    {
        // PolishRegon owns its own detector branch in the 9-digit bucket and the
        // 14-digit bucket. Iter-3 noted neither had positive coverage.
        var result = _detector.Detect(raw);
        result.Should().BeOfType<PolishRegon>();
        result!.CountryCode.Should().Be("PL");
    }

    [Theory]
    [InlineData("104052135")]  // NIS a.d. PIB (iter-3: SerbianPib positive cover requested)
    public void Detect_SerbianPib_ReturnsSerbia_Detailed(string raw)
    {
        var result = _detector.Detect(raw);
    ...
    [Theory]
    [InlineData("FI0112038-9")]   // Finnish prefix
    [InlineData("fi 0112038-9")]  // mixed-case prefix + space
    public void Detect_FinnishWithExplicitPrefix_ReturnsFinland(string raw)
    {
        var result = _detector.Detect(raw);
        result.Should().BeOfType<FinnishBusinessId>();
    }
    ...
    // Iter-3 caught that the previous "iter-2 collision case" 5560125790 doesn't actually
    // pass PL NIP MOD-11 (sum=238, expected 7, actual 0). The regression test value for
    // the SE→PL-NIP misroute is therefore weaker than intended: the bucket ordering
    // (SE before PL NIP) protects against the iter-1 critical regardless, but a future
    // re-order won't be caught by these test values alone. A genuine SE-Luhn + PL-NIP
    // collision would need to be generated by brute force — left as a known test-gap.
