...
public sealed class WebApiFactory : WebApplicationFactory<Program>
{
    ...
    {
        // The CompanyProviderRegistry constructor reads CountryCode + IsConfigured from
        // each registered ICompanyProvider. NSubstitute default-returns null/false for
        // those getters, so seed sane defaults here. Tests that need a different country
        // or "not configured" state can override per-instance.
        Company.CountryCode.Returns("NO");
        Company.IsConfigured.Returns(true);
    }
